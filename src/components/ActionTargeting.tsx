...
export default function ActionTargeting({ pendingAction }: Props) {
  ...
  if (pendingAction.type === 'rotate-goals') {
    ...
    const rawRing = effectiveSeatOrder.map(seat => {
      if (seat !== null) {
        return { label: seat === myIdx ? t('right.you') : state.players[seat].name, isMe: seat === myIdx, isPlayer: true };
      } else {
        unusedCursor++;
    ...
    return (
      <div className="absolute inset-0 flex items-center justify-center z-40 bg-black/60 backdrop-blur-sm">
        <div className="bg-zinc-900 rounded-2xl p-5 border border-white/20 shadow-2xl text-center max-w-sm w-full mx-4">
          <p className="text-white font-bold text-lg mb-1">{t('targeting.rotateGoals')}</p>
          <p className="text-white/40 text-xs mb-4">{t('targeting.chooseDirection')}</p>
          ...
          <div className="flex gap-3">
            ...
            >
              <span>{t('targeting.left')}</span>
              <span className="text-blue-200/70 text-[10px] font-normal leading-tight">
                {t('targeting.receive', { label: leftSource.label })}
              </span>
            </button>
            <button
              ...
              className="flex-1 py-3 px-2 bg-blue-700/80 hover:bg-blue-600 text-white font-bold rounded-xl transition-colors flex flex-col items-center gap-0.5"
            >
              <span>{t('targeting.right')}</span>
              <span className="text-blue-200/70 text-[10px] font-normal leading-tight">
                {t('targeting.receive', { label: rightSource.label })}
              </span>
            </button>
          </div>
        </div>
    ...
    return (
      <div className="absolute inset-0 flex items-center justify-center z-40 bg-black/60 backdrop-blur-sm">
        <div className="bg-zinc-900 rounded-2xl p-8 border border-white/20 shadow-2xl text-center max-w-xs w-full mx-4">
          <p className="text-white font-bold text-lg mb-2">{t('targeting.tradeHands')}</p>
          <p className="text-white/50 text-sm mb-6">{t('targeting.choosePlayerHands')}</p>
          <div className="flex flex-col gap-3">
            {otherPlayers.map(p => {
              ...
              return (
                ...
                >
                  {p.name} ({t('targeting.nCards', { count })})
                </button>
              );
            })}
          </div>
  ...
  if (pendingAction.type === 'trade-goals') {
    ...
    return (
      <div className="absolute inset-0 flex items-center justify-center z-40 bg-black/60 backdrop-blur-sm">
        <div className="bg-zinc-900 rounded-2xl p-8 border border-white/20 shadow-2xl text-center max-w-xs w-full mx-4">
          <p className="text-white font-bold text-lg mb-2">{t('targeting.tradeGoals')}</p>
          <p className="text-white/50 text-sm mb-4">{t('targeting.choosePlayerGoals')}</p>
          <div className="flex flex-col gap-3">
            {otherPlayers.map(p => (
              <button
                key={p.id}
            ...
            {unusedGoals.map((g, i) => (
              ...
              >
                {t('right.unusedGoal', { n: state.players.length + i + 1 })}
              </button>
            ))}
          </div>
        </div>
  ...
  if (pendingAction.type === 'zap-card' || pendingAction.type === 'move-card') {
    ...
    const label = needsDest
      ? t('targeting.clickZone')
      : isMove
        ? t('targeting.clickToMove')
        : t('targeting.clickToZap');
